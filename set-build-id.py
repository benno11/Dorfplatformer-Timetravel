...
Usage:
  python3 build/set-build-id.py <build_number>
"""
...
def resolve_config_path() -> Path:
    ...
    candidate = project_root / "assets" / "config.json"
    if candidate.exists():
        return candidate
...
def main() -> int:
    if len(sys.argv) < 2:
        print("Usage: python3 build/set-build-id.py <build_number>", file=sys.stderr)
        return 2
