...
#   SDL_MIXER_REF=main RECOMPILE_PC=1 ./build/recompile-sdl.sh
...
export ABI API JOBS SDL_REF SDL_MIXER_REF SDL_REQUIRED_VERSION FORCE_REBUILD_SDL
...
if [ "$RECOMPILE_PC" = "1" ]; then
  echo "[INFO] Recompiling SDL for PC/Linux (including SDL_mixer)"
  SDL3_REQUIRED_VERSION="$SDL_REQUIRED_VERSION" \
  SDL3_SOURCE_REF="$SDL_REF" \
  SDL3_MIXER_SOURCE_REF="$SDL_MIXER_REF" \
  ...
  if [ ! -f "$ROOT_DIR/deps/linux-sdl3-mixer-src/build/sdl3-mixer.pc" ] && [ ! -f "/usr/local/lib/pkgconfig/sdl3-mixer.pc" ] && [ ! -f "/usr/lib/pkgconfig/sdl3-mixer.pc" ]; then
    echo "[ERROR] PC SDL rebuild did not produce sdl3-mixer.pc"
    echo "        Checked:"
    echo "        $ROOT_DIR/deps/linux-sdl3-mixer-src/build/sdl3-mixer.pc"
    echo "        /usr/local/lib/pkgconfig/sdl3-mixer.pc"
    echo "        /usr/lib/pkgconfig/sdl3-mixer.pc"
    exit 1
  fi
fi
